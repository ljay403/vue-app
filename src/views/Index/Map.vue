<template>
  <baidu-map
    class="map"
    ak="Ps7d8xotpfXbGNyjljFhqoPH52dCGHjv"
    :center="center"
    :zoom="zoom"
  ...
  >
    <bm-marker v-for="(x, i) in ohtersInfo" :position="{lng: x.lon, lat: x.lat}" :key="i">
      <bm-label
        ...
        :offset="{width: -(x.name.length-1)/2*12, height: 35}"
      />
    </bm-marker>
    <bm-marker :position="{lng: userInfo.lon, lat: userInfo.lat}">
      <bm-label
        :content="userInfo.name"
        :labelStyle="{color: 'blue',border: '1px solid blue', fontSize : '12px'}"
        :offset="{width: -(userInfo.name.length-1)/2*12, height: 35}"
      />
    </bm-marker>
  </baidu-map>
</template>
